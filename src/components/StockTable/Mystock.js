...
class Mystock extends Component {
  ...
  componentDidMount() {
    axios
      .get(`https://financial-portfolio-30240.firebaseio.com/stock1.json`)
      .then((db) => {
        const res = db.data;
        if (res === null) {
          this.setState({
          ...
          for (let i = 0; i < Keys.length; i++) {
            ...
            this.StockInfo = (x) => {
              axios
                .get(
                  `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${c_symb}&apikey=DJ8FLXWOM0J112Q0`
                )
                .then((res) => {
                  let Data = [];
                  let Current_p = [];
  ...
  stopTrack = (index) => {
    ...
    let ref = firebase.database().ref("stock1");
    ref
